...
USER_AGENT = "WebactueelSEOChecker/1.0 (+https://github.com/Yolol100/seochecker)"
...
def analyze_html(html: str, base_url: str) -> dict:
    ...
    canonical = [urljoin(base_url, link.get("href", "")) for link in parser.links if "canonical" in link.get("rel", "").lower().split() and link.get("href")]
    ...
    for raw in parser.jsonld_raw:
        try:
            ...
            jsonld_types.update(_jsonld_types(parsed))
...
def probe(url: str, timeout: int = 10) -> dict:
    try:
        status, final_url, headers, body = fetch(url, timeout=timeout)
...
def run(url: str) -> dict:
    ...
    analysis = analyze_html(page.get("body", ""), final_url)
    result.update(analysis)
    if page["status"] < 200 or page["status"] >= 300:
        result["indexability_blockers"].append(f"pagina retourneert HTTP {page['status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Small evidence-first technical SEO check")
