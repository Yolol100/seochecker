...
class PageParser(HTMLParser):
    def __init__(self):
        super().__init__(convert_charrefs=True); self.in_title=False; self.in_h1=False; self.in_jsonld=False
        self.title_parts=[]; self.h1_parts=[]; self.h1s=[]; self.meta=[]; self.links=[]; self.jsonld_raw=[]; self._jsonld_parts=[]
    def handle_starttag(self, tag, attrs):
        attrs={str(k).lower():(v or "") for k,v in attrs}; tag=tag.lower()
        if tag=="title": self.in_title=True
        elif tag=="h1": self.in_h1=True; self.h1_parts=[]
        elif tag=="meta": self.meta.append(attrs)
        elif tag=="link": self.links.append(attrs)
        elif tag=="script" and attrs.get("type","").lower()=="application/ld+json": self.in_jsonld=True; self._jsonld_parts=[]
    def handle_endtag(self, tag):
        tag=tag.lower()
        if tag=="title": self.in_title=False
        elif tag=="h1":
            self.in_h1=False; text=" ".join("".join(self.h1_parts).split())
            if text: self.h1s.append(text)
        elif tag=="script" and self.in_jsonld:
            self.in_jsonld=False; raw="".join(self._jsonld_parts).strip()
...
def _meta_values(parser,key,value): return [m.get("content","").strip() for m in parser.meta if m.get(key,"").lower()==value and m.get("content")]
...
def _normalize_url(value):
    parsed=urlparse(value); path=parsed.path or "/"
...
def analyze_html(html,base_url):
    ...
    title=" ".join("".join(parser.title_parts).split()); descriptions=_meta_values(parser,"name","description")
    robots=_meta_values(parser,"name","robots"); googlebot=_meta_values(parser,"name","googlebot")
    canonical=[urljoin(base_url,l.get("href","")) for l in parser.links if "canonical" in l.get("rel","").lower().split() and l.get("href")]
    ...
    if len(title)>65: warnings.append("title is langer dan 65 tekens")
    ...
    if descriptions and len(descriptions[0])>160: warnings.append("meta description is langer dan 160 tekens")
    if len(parser.h1s)==0: warnings.append("H1 ontbreekt")
    elif len(parser.h1s)>1: warnings.append("meerdere H1's gevonden")
    ...
    return {"title":title,"title_length":len(title),"meta_descriptions":descriptions,"robots":robots,"googlebot":googlebot,"h1":parser.h1s,"canonical":canonical,"canonical_self_referencing":len(canonical)==1 and _normalize_url(canonical[0])==_normalize_url(base_url),"hreflang":hreflang,"hreflang_duplicate_languages":duplicates,"hreflang_invalid_languages":invalid,"jsonld_blocks":len(parser.jsonld_raw),"jsonld_types":sorted(jsonld_types),"jsonld_errors":jsonld_errors,"indexability_blockers":blockers,"warnings":warnings}

def fetch(url,timeout=20):
    req=Request(url,headers={"User-Agent":USER_AGENT,"Accept":"text/html,application/xhtml+xml,*/*;q=0.8"})
    with urlopen(req,timeout=timeout) as response:
        body=response.read(8_000_000); charset=response.headers.get_content_charset() or "utf-8"
        return response.status,response.geturl(),response.headers,body.decode(charset,errors="replace")
def probe(url,timeout=10):
    try:
        status,final_url,headers,body=fetch(url,timeout); return {"url":url,"status":status,"final_url":final_url,"content_type":headers.get("Content-Type",""),"body":body}
    except HTTPError as exc: return {"url":url,"status":exc.code,"final_url":exc.geturl(),"error":str(exc)}
    except (URLError,TimeoutError,OSError) as exc: return {"url":url,"status":None,"error":str(exc)}
def run(url):
    page=probe(url,25); result={"requested_url":url,"http":{k:v for k,v in page.items() if k!="body"}}
    ...
    final_url=page["final_url"]; result.update(analyze_html(page.get("body",""),final_url))
    origin=f"{urlparse(final_url).scheme}://{urlparse(final_url).netloc}"; robots_url=urljoin(origin+"/","robots.txt"); robots=probe(robots_url); robots_body=robots.pop("body","")
    sitemap_urls=[]
    ...
    sitemap=probe(sitemap_urls[0]); sitemap.pop("body",None); result["robots_txt"]=robots; result["sitemap_candidates"]=sitemap_urls; result["sitemap_probe"]=sitemap
    return result
def main():
