...
The safe core also records a **Googlebot Search fetch-prefix** observation for HTML/text responses. Current Google Search Central documentation (checked 2026-09-18) says Googlebot crawls the first 2 MB of supported non-PDF file types and the first 64 MB of PDFs, with the file-size limit applied to uncompressed data. The checker requests identity encoding and bounds the HTML body at 2,000,000 bytes; it reports whether more response-body bytes exist beyond that point. This is a conservative response-body observation: HTTP header bytes are not counted by the checker, so it is not represented as a byte-for-byte emulation of Google's crawler.
