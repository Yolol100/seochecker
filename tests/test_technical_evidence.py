...
class TechnicalEvidenceTests(unittest.TestCase):
    ...
    def test_sitemap_reconciliation(self):
        ...
        graph = validate_graph(normalized, ["https://example.com/a", "https://example.com/missing"])
        kinds = {x["type"] for x in graph["issues"]}
        self.assertIn("sitemap_url_not_observed", kinds)
        self.assertIn("indexable_url_missing_from_sitemap", kinds)
